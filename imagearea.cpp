...
void ImageArea::open()
{
    ...
    QFileDialog dialog(this, tr("Відкрити зображення"), "", mOpenFilter);
...
void ImageArea::open(const QString &filePath)
{
    ...
    else
    {
        ...
        QApplication::restoreOverrideCursor();
        QMessageBox::warning(this, tr("Помилка відкриття файлу"), tr("Не вдається відкрити файл \"%1\".").arg(filePath));
    }
}
...
bool ImageArea::save()
{
    ...
    {
        QMessageBox::warning(this, tr("Помилка збереження файлу"), tr("Не вдається зберегти файл \"%1\".").arg(mFilePath));
        mOriginalImage = *mImage;
        return false;
    }
    mIsEdited = false;
...
bool ImageArea::saveAs()
{
    ...
    if(fileName.isEmpty())
    {
        fileName = QDir::homePath() + "/" + tr("Untitled") + ".png";
    }
    QString filePath = QFileDialog::getSaveFileName(this, tr("Зберегти зображення"), fileName, mSaveFilter,
                                                    &filter,
    ...
    if(!filePath.isEmpty())
    {
        ...
        {
            QMessageBox::warning(this, tr("Помилка збереження файлу"), tr("Не вдається зберегти файл \"%1\".").arg(filePath));
            result = false;
        }
    }
    QApplication::restoreOverrideCursor();
